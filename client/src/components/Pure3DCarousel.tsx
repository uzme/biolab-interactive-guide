...
export default function Pure3DCarousel({ onSelectDevice }: Pure3DCarouselProps) {
  ...
  const pageEquipment = equipment.slice(
    currentIndex * pageSize,
    (currentIndex + 1) * pageSize,
  );

  return (
    <section className="pure3d-carousel my-8 rounded-[32px] border border-[#d3e5df] bg-gradient-to-b from-[#f7fcfb] to-[#edf7f4] p-6 shadow-[0_20px_50px_rgba(28,71,67,0.06)] sm:p-10" aria-label="Asosiy qurilmalar 3D carouseli">
      ...
      {pageEquipment.length > 0 ? (
        <div className="scene" aria-live="polite">
          <div
            className="a3d"
            style={{ "--n": pageEquipment.length } as CarouselStyle}
          >
