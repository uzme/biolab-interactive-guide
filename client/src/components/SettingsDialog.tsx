        ...
        <div className="max-h-[calc(92vh-122px)] overflow-y-auto px-5 py-5 sm:px-7 sm:py-6">
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <section className="rounded-2xl border border-[#cfe4db] bg-white p-5 shadow-[0_8px_20px_rgba(23,61,66,0.04)]">
              ...
              <div className="mt-4 rounded-xl border border-[#dcebe5] bg-[#f7fbfa] p-3"><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-bold text-[#173d42]">Saqlangan qurilmalar</div><div className="mt-1 text-xs text-[#68857f]">{bookmarkedCount} ta xatcho‘p faol</div></div><button type="button" onClick={handleClearBookmarks} aria-label="Xatcho‘plarni tozalash" className="inline-flex items-center gap-1.5 rounded-lg border border-[#efc8c2] bg-white px-3 py-2 text-xs font-bold text-[#b44d43] transition hover:bg-[#fff3f1]"><Trash2 size={14} /> Tozalash</button></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={onExportBookmarks} className="inline-flex items-center gap-1.5 rounded-lg border border-[#b8d8ce] bg-white px-3 py-2 text-xs font-bold text-[#0d7774] transition hover:bg-[#e5f3ed]">JSON eksport</button><button type="button" onClick={() => importInputRef.current?.click()} className="inline-flex items-center gap-1.5 rounded-lg border border-[#b8d8ce] bg-white px-3 py-2 text-xs font-bold text-[#0d7774] transition hover:bg-[#e5f3ed]">JSON import</button><input ref={importInputRef} type="file" accept="application/json,.json" className="hidden" aria-label="Xatcho‘plar JSON faylini tanlash" onChange={(event) => { const file = event.target.files?.[0]; if (file) void handleImportBookmarks(file); event.target.value = ""; }} /></div></div>
            </section>
          </div>
          ...
          <section className="mt-4 rounded-2xl border border-[#cfe4db] bg-white p-5" data-settings-licensing>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-[#d8e7e3] bg-[#f7fbfa] p-3"><div className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#0d7774]">Kod va interfeys</div><div className="mt-1 text-sm font-bold text-[#173d42]">Loyiha litsenziyasi</div></div><div className="rounded-xl border border-[#d8e7e3] bg-[#f7fbfa] p-3"><div className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#0d7774]">Rasm manbasi</div><div className="mt-1 text-sm font-bold text-[#173d42]">Har bir dosyeda ko‘rsatiladi</div></div><div className="rounded-xl border border-[#d8e7e3] bg-[#f7fbfa] p-3"><div className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#0d7774]">Copyright</div><div className="mt-1 text-sm font-bold text-[#173d42]">© 2026 BioLab</div></div></div>
          </section>
