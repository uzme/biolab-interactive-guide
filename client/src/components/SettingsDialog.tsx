        ...
        <div className="max-h-[calc(100dvh-154px)] overflow-y-auto overscroll-contain px-5 py-5 [-webkit-overflow-scrolling:touch] sm:px-7 sm:py-6">
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <section className="rounded-2xl border border-[#cfe4db] bg-white p-5 shadow-[0_8px_20px_rgba(23,61,66,0.04)]">
              ...
              <div className="mt-4 rounded-xl border border-[#dcebe5] bg-[#f7fbfa] p-3"><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-bold text-[#173d42]">Saqlangan qurilmalar</div><div className="mt-1 text-xs text-[#68857f]">{bookmarkedCount} ta xatcho‘p faol</div></div><button type="button" onClick={handleClearBookmarks} aria-label="Xatcho‘plarni tozalash" className="inline-flex items-center gap-1.5 rounded-lg border border-[#efc8c2] bg-white px-3 py-2 text-xs font-bold text-[#b44d43] transition hover:bg-[#fff3f1]"><Trash2 size={14} /> Tozalash</button></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={onExportBookmarksCsv} disabled={bookmarkedCount === 0} className="inline-flex items-center gap-1.5 rounded-lg border border-[#b8d8ce] bg-white px-3 py-2 text-xs font-bold text-[#0d7774] transition hover:bg-[#e5f3ed] disabled:cursor-not-allowed disabled:opacity-45">CSV eksport</button><button type="button" onClick={() => { void onExportBookmarksPdf(); }} disabled={bookmarkedCount === 0} className="inline-flex items-center gap-1.5 rounded-lg border border-[#b8d8ce] bg-white px-3 py-2 text-xs font-bold text-[#0d7774] transition hover:bg-[#e5f3ed] disabled:cursor-not-allowed disabled:opacity-45">PDF eksport</button><button type="button" onClick={() => { void onShareBookmarksPdf(); }} disabled={bookmarkedCount === 0} className="inline-flex items-center gap-1.5 rounded-lg border border-[#b8d8ce] bg-white px-3 py-2 text-xs font-bold text-[#0d7774] transition hover:bg-[#e5f3ed] disabled:cursor-not-allowed disabled:opacity-45">PDF ulashish</button><button type="button" onClick={onExportBookmarks} className="inline-flex items-center gap-1.5 rounded-lg border border-[#b8d8ce] bg-white px-3 py-2 text-xs font-bold text-[#0d7774] transition hover:bg-[#e5f3ed]">JSON eksport</button><button type="button" onClick={() => importInputRef.current?.click()} className="inline-flex items-center gap-1.5 rounded-lg border border-[#b8d8ce] bg-white px-3 py-2 text-xs font-bold text-[#0d7774] transition hover:bg-[#e5f3ed]">JSON import</button><input ref={importInputRef} type="file" accept="application/json,.json" className="hidden" aria-label="Xatcho‘plar JSON faylini tanlash" onChange={(event) => { const file = event.target.files?.[0]; if (file) void handleImportBookmarks(file); event.target.value = ""; }} /></div></div>
            </section>
          </div>
