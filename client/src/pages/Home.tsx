...
function Sidebar({ activeCategory, onCategory, onMobileClose }: { activeCategory: string; onCategory: (category: string) => void; onMobileClose?: () => void }) {
  return <aside className="sidebar">
    <div className="mb-10 flex items-center gap-3 px-2">
      <div className="brand-mark"><img src="/manus-storage/biolab-logo_c6e5d846.png" alt="BioLab belgisi" className="h-7 w-7 object-contain" /></div>
      <div className="sidebar-copy"><div className="display text-xl font-bold text-[#173d42]">BioLab</div><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#6d8b87]">Bilim katalogi</div></div>
    </div>
    ...
    <main className="min-w-0 flex-1">
      ...
      <div className="mx-auto max-w-[1500px] px-5 py-6 sm:px-8 sm:py-10 lg:px-12">
        <section className="soft-grid relative overflow-hidden rounded-[30px] bg-[#123f46] shadow-[0_25px_60px_rgba(26,76,72,0.14)]">
          ...
          <div className="absolute right-7 top-7 hidden w-[205px] rounded-2xl border border-white/15 bg-[#0a3d45]/72 p-4 text-[#d1f1e6] backdrop-blur-md lg:block"><div className="tech-label mb-3 text-[#9fe8d5]">Live learning surface</div><div className="mb-3 flex items-center gap-2 text-xs font-semibold"><span className="h-2 w-2 rounded-full bg-[#7de0c2]" /> 3D model ready</div><div className="space-y-2 text-[11px] text-[#b6dbd1]"><div className="flex items-center justify-between"><span>Hotspotlar</span><span className="font-bold text-white">03</span></div><div className="flex items-center justify-between"><span>Guided tour</span><span className="font-bold text-white">UZ</span></div><div className="signal-line mt-3" /></div></div>
        ...
        <section className="mt-8" aria-live="polite"><div className="mb-4 flex items-center justify-between"><div><h2 className="display text-2xl font-bold text-[#173d42]">{activeCategory}</h2><p className="mt-1 text-sm text-[#78908c]">{filtered.length} ta qurilma topildi</p></div>{activeCategory !== "Barcha uskunalar" && <button onClick={() => setActiveCategory("Barcha uskunalar")} className="text-xs font-bold text-[#0d7774] hover:underline">Barchasini ko‘rsatish</button>}</div>
          {filtered.length > 0 ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.slice(0, 18).map((device, index) => <article key={device.id} className="equipment-card group rounded-[22px] border border-[#d8e7e3] bg-[#ffffff] p-5" style={{ animationDelay: `${Math.min(index, 7) * 35}ms` }}><div className="mb-5 flex items-start justify-between gap-3"><div className="relative grid h-14 w-20 place-items-center overflow-hidden rounded-xl border border-[#cce2dc] bg-[#f1f8f6] soft-grid"><FlaskConical size={21} className="text-[#0b7972]" /><span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-[#54cdb1] shadow-[0_0_0_4px_rgba(84,205,177,0.18)]" /></div><span className="rounded-full bg-[#eaf5f2] px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-[#6c8e86]">BIO-{String(device.number).padStart(3, "0")}</span></div><div className="eyebrow mb-2">{device.category}</div><h3 className="display min-h-[58px] text-xl font-bold leading-tight text-[#173d42]">{device.name}</h3><div className="mt-3 flex flex-wrap gap-1.5"><span className="tech-label rounded-md bg-[#f0f6f5] px-2 py-1 text-[#4e7771]">Model: {device.model.slice(0, 18)}</span><span className="tech-label rounded-md bg-[#e4f4ed] px-2 py-1 text-[#0d7774]">Hotspot</span></div><p className="mt-3 min-h-[54px] text-sm leading-6 text-[#6f8984]">{device.description}</p><div className="mt-5 flex items-center justify-between border-t border-[#e5eee8] pt-4"><span className="max-w-[160px] truncate text-xs font-semibold text-[#78908c]">{device.newPrice}</span><Button size="sm" variant="outline" className="border-[#b9d8cd] bg-transparent text-[#0d7774] hover:bg-[#e3f2e9]" onClick={() => setSelectedDevice(device)}>O‘rganish <ChevronRight size={15} /></Button></div></article>)}</div> : <div className="rounded-[22px] border border-dashed border-[#b9d8cd] bg-[#ffffff] px-6 py-16 text-center"><Search size={28} className="mx-auto mb-4 text-[#70a298]" /><h3 className="display text-2xl font-bold">Qurilma topilmadi</h3><p className="mt-2 text-sm text-[#78908c]">Qidiruv so‘zini yoki kategoriyani o‘zgartirib ko‘ring.</p></div>}
