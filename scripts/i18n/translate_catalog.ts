...
const concurrency = Number(process.env.BIOLAB_TRANSLATION_CONCURRENCY || 3);
const onlyId = process.env.BIOLAB_TRANSLATION_ONLY;
...
function translatedInput(device: typeof equipment[number], learning: Record<string, unknown> | null, purchase: Record<string, unknown> | null) {
  return { equipment: { name: device.name, category: device.category, description: device.description, purpose: device.purpose, hiddenCosts: device.hiddenCosts, purchasingStrategy: device.purchasingStrategy }, learning, purchase };
}
function cleanLearning(value: Record<string, unknown> | undefined) { if (!value) return null; const { number: _number, sourceKind: _sourceKind, ...rest } = value; return rest; }
function cleanPurchase(value: Record<string, unknown> | undefined) { if (!value) return null; const { number: _number, sourceKind: _sourceKind, ...rest } = value; return rest; }
...
async function translateOne(device: typeof equipment[number]): Promise<TranslationRecord> {
  ...
  const prompt = [
    ...
    JSON.stringify(translatedInput(device, learning, purchase)),
  ].join("\n\n");
  for (let attempt = 1; attempt <= 5; attempt += 1) {
    const response = await fetch(`${process.env.OPENAI_API_BASE}/chat/completions`, {
      method: "POST",
      headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}`, "Content-Type": "application/json" },
      body: JSON.stringify({ model, messages: [{ role: "system", content: "You are a senior scientific translator specializing in laboratory instrumentation, biosafety, molecular biology and technical documentation. Output valid JSON only." }, { role: "user", content: prompt }], max_completion_tokens: 18000, response_format: { type: "json_object" } }),
    });
    if (!response.ok) { if (attempt === 5) throw new Error(`${device.id}: ${response.status} ${await response.text()}`); await new Promise((resolve) => setTimeout(resolve, attempt * 2000)); continue; }
    const payload = await response.json() as { choices?: Array<{ message?: { content?: string } }> };
    const content = payload.choices?.[0]?.message?.content?.trim();
    if (content) {
      try {
        const parsed = JSON.parse(content) as TranslationRecord;
        if (parsed.en?.equipment && parsed.ru?.equipment) return { id: device.id, en: parsed.en, ru: parsed.ru };
      } catch { /* retry malformed JSON */ }
    }
    if (attempt < 5) await new Promise((resolve) => setTimeout(resolve, attempt * 2000));
  }
  throw new Error(`${device.id}: empty or invalid model response after retries`);
}
...
async function main() {
  ...
  const pending = equipment.filter((device) => !done.has(device.id) && (!onlyId || device.id === onlyId));
