...
try {
  for (const { deviceIndex, block } of testCases) {
    ...
    await assert(await loadingHeading.evaluate((heading) => heading.closest("[role='status']")?.getAttribute("aria-live") === "polite"), `Learning block ${block}: loading status semantikasi ko‘rinmadi.`);
