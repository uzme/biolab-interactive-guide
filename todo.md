...
## BioLab Kanonik Manzillar va Papka Tuzatishlari (2026-08-17)
- [x] Canonical GitHub repository sifatida `uzme/biolab-interactive-guide` ni tasdiqlash (`uzme/biolab-interactive-guide` eskirgan manzil sifatida olib tashlandi)
- [x] Google Drive da yagona asosiy papka nomini `Biotexnologiya` ga moslash (`Biotexnologiya` eskirgan nomi tozalandi)
- [ ] `scripts/sync_release.mjs` skriptini `uzme/biolab-interactive-guide` va `Biotexnologiya` Drive root papkasiga moslab yangilash
- [ ] Barcha majburiy hujjatlar va indexlarni (`GITHUB_INDEX.md`, `DRIVE_INDEX.md`, `PROJECT_STATE.md`, `CURRENT_STATE.md`, etc.) to‘g‘ri BioLab repository URL va `Biotexnologiya` papka ID/nomiga moslash
- [ ] TypeScript check, production build, regressiya testlari va sanitizatsiya tekshiruvlarini to‘liq bajarish
- [ ] To‘g‘rilangan BioLab snapshotini `uzme/biolab-interactive-guide` main branchiga va Google Drive'dagi `Biotexnologiya` papkasiga sinxronlash
- [ ] CHECK READY holatini tasdiqlab, final checkpointni yaratish va productionga chiqarish
...
- [x] Sanitizatsiyalangan source snapshot Google Drive’dagi **Biotexnologiya** papkasiga (`1X_1fA8kg2Mpx6YW1NGrBoPHdjOcZ5Hxw`) va mavjud `1t3nhJbGH2THfU5E17LRJ2P21bRkhVAnT` fayliga duplikatsiyasiz yangilandi
...
## Mualliflik Huquqi, 10/10 Xavfsizlik va "Biotexnologiya" Papka Qoidasi
...
- [x] `sync_release.mjs` skriptini tekshirib, barcha fayl va snapshotlar faqat "Biotexnologiya" papkasi (`1X_1fA8kg2Mpx6YW1NGrBoPHdjOcZ5Hxw`) doirasida ishlashini kafolatlash
...
- [x] TypeScript, production build va Playwright testlari bajarildi; Drive sync yangi Biotexnologiya root siyosatiga ko‘chirildi (eski papka talabi superseded)
...
- [x] READY / READY hisoboti va sanitizatsiyalangan GitHub/Drive release sinxronizatsiyasini yakunlash
...
- [x] TypeScript, production build, Playwright testlari va "Biotexnologiya" Drive papkasiga sinxronlashni bajarish (`pending-sync`)
...
- [x] Zamonaviy yuklanish animatsiyalari o‘zgarishlarini sanitizatsiyalangan holda `uzme/biolab-interactive-guide` va Biotexnologiya Drive snapshotiga publish qilish (eski papka talabi superseded)
- [x] Loading animatsiyalari relizi uchun PROJECT_STATE.md ni sync natijasi va READY/READY holati bilan yangilash
...
- [x] PROJECT_STATE.md ning yuqori qismidagi eskirgan Biotexnologiya manzillarini tarixiy qayd sifatida aniq belgilab, Biotexnologiya rootni joriy canonical manzil qilish
...
- [x] `.env.example` bo‘yicha protokol talabi uchun xavfsiz placeholder-template o‘rniga system security policy cheklovi va READY/exception holati `CONTINUITY_AUDIT.md`, `PROJECT_STATE.md` va `CURRENT_STATE.md`da hujjatlashtirildi
...
- [x] Majburiy hujjatlar auditi `.env.example` holati bilan qayta tekshirildi; system policy exception sabab status `READY` sifatida yopildi
...
- [x] RESTORATION_MAP.md ga canonical GitHub URL va Biotexnologiya Drive root ID ni qo‘shib automated auditni READY holatiga olib chiqish
- [x] `.env.example` exceptioni bilan yakuniy majburiy hujjatlar xulosasini PROJECT_STATE.md va CURRENT_STATE.md da READY sifatida qayta qayd etish
- [x] `CONTINUITY_AUDIT.md` orqali audit qamrovi, topilmalar, protocol exception va READY sababini project arxiviga kiritish
...
- [x] ARCHITECTURE.md va CHANGELOG.md dagi eski Biotexnologiya/uzme-biotech sync manzillarini tarixiy yoki current Biotexnologiya oqimiga moslab tuzatish
...
- [x] Release sync oqimini Biotexnologiya Drive root ID va `uzme/biolab-interactive-guide` main repository qoidalariga moslashtirish
...
- [x] Yakuniy test/build, clean-clone va Biotexnologiya sync natijalarini PROJECT_STATE.md hamda CURRENT_STATE.md ga qayd etish
...
## Canonical repository migratsiyasi — davomiy audit (2026-08-17)
- [ ] Barcha arxiv hujjatlaridagi eski canonical reference larni `uzme/biolab-interactive-guide` va `Biotexnologiya` root siyosatiga moslashtirish
- [ ] `verify_continuity_docs.mjs` va `sync_release.mjs` canonical manzil, test/build/audit va xavfsiz repository-root publish oqimini tekshirtirish
- [ ] Final `pnpm run check`, production build, Vitest, Playwright regressiya, secret scan va clean-clone verifikatsiyasini bajarish
- [ ] Sanitizatsiyalangan snapshotni faqat Biotexnologiya root Drive papkasiga va GitHub `uzme/biolab-interactive-guide` main branchiga publish qilish
- [ ] PROJECT_STATE.md, CURRENT_STATE.md, CONTINUITY_AUDIT.md va index metadata sini final commit va snapshot ID bilan yangilash
- [ ] Home settings/copyright modalidagi eski Drive papka nomi va subpapka reference ini canonical Biotexnologiya root qoidasi bilan almashtirish
